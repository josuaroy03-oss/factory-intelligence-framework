...
def plot_factory_floor(coords: np.ndarray, D: np.ndarray) -> go.Figure:
    ...
    for i, j in combinations(range(n), 2):
        ...
        fig.add_trace(go.Scatter(
            ...
            hoverinfo="text",
            text=f"{STATION_NAMES[i]} ↔ {STATION_NAMES[j]}: {d:.2f} units",
            showlegend=False,
        ))
    ...
    fig.update_layout(
        title=dict(
            ...
            x=0.5, 
            xanchor='center' # <-- PERBAIKAN DI SINI
        ), 
...
def plot_distance_heatmap(D: np.ndarray) -> go.Figure:
    ...
    fig = go.Figure(data=go.Heatmap(
        ...
        hovertemplate="Dari: %{y}<br>Ke: %{x}<br>Jarak: %{z:.2f} unit<extra></extra>",
        colorbar=dict(
            title=dict(text="Jarak (unit)", font=dict(color="#8b949e", size=14)),
...
def render_module1():
    ...
    st.markdown(
        ...
        <div style="text-align: center; padding: 1.5rem; background-color: #161b22; border-radius: 8px; border: 1px solid #30363d; margin-bottom: 1.5rem;">
            ...
            <div style="color: #f0a500; font-size: 2.8rem; font-weight: bold; font-family: sans-serif;">
                {total_efficiency:.2f} <span style="font-size: 1.5rem; color: #e6edf3;">unit</span>
            </div>
        </div>
        """,
    ...
    if total_efficiency < 60:
        st.success(f"**Status: SANGAT EFISIEN** ✅\n\nTotal perpindahan material berada di angka **{total_efficiency:.2f} unit**. Tata letak saat ini sudah sangat optimal. Stasiun kerja saling berdekatan sehingga meminimalkan biaya logistik internal.")
    elif total_efficiency < 85:
        st.info(f"**Status: CUKUP EFISIEN** ⚠️\n\nTotal perpindahan material adalah **{total_efficiency:.2f} unit**. Tata letak ini dapat diterima, namun masih ada ruang untuk dioptimalkan dengan cara mendekatkan stasiun-stasiun yang letaknya berjauhan.")
    else:
        st.error(f"**Status: TIDAK EFISIEN** ❌\n\nTotal perpindahan material mencapai **{total_efficiency:.2f} unit**! Tata letak ini terlalu menyebar. Sangat disarankan untuk merombak posisi agar stasiun utama lebih berdekatan satu sama lain demi menghemat waktu perakitan.")
