...
def render_module2():
    ...
    st.caption("📋 **Acuan Standar Klasifikasi Metrik:**\n"
               "- **Indikator Hubungan (Determinan):** `≈ 0.00` (Sistem Kaku/Bergantung) | `≠ 0.00` (Sistem Fleksibel/Independen)\n"
               "- **Intensitas Matematis (Nilai Eigen):** `< 10` (Sirkulasi Rendah) | `10 - 39.9` (Sirkulasi Sedang) | `≥ 40` (Sirkulasi Tinggi)\n"
               "- **Total Aliran Harian:** `< 100` unit (Rendah) | `100 - 250` unit (Sedang) | `> 250` unit (Tinggi)\n"
               "- **Rute Aliran Aktif:** Jumlah jalur dengan aliran material aktif (maksimal 25 rute berpotensi dalam matriks 5x5).")
    ...
    with st.expander("📊 Lihat Daftar Kategori Hubungan Antar-Stasiun", expanded=False):
        ...
        if relationship_data:
            ...
            st.caption("Aturan Klasifikasi: Lemah (1-15 unit), Sedang (16-30 unit), Kuat (>30 unit)")
        else:
